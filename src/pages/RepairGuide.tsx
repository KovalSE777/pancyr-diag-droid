...
const repairData: RepairItem[] = [
  ...
  // КОМПРЕССОР
  {
    ...
    category: 'compressor',
    severity: 'warning',
    symptoms: ['Компрессор не запускается', 'Нет охлаждения', 'Индикация ошибки "отказ компрессора" связано либо с вентилятором компрессора либо с внутренними процессами УПП'],
    diagnosis: 'Подключить приложение по диагностике. Проверить последовательность сигналов УПП (устройство плавного пуска): 1) SVD - сигнал с фотодиода, 2) Контакт норма.',
    solution: [
      'Проверить питание 380В на жгуте компрессора',
      'Принудительно запускаем компрессор через приложение',
    ...
    tools: ['Шуруповерт', 'Набор инструментов', 'Схема подключения']
  },
  // ДАТЧИКИ
  {
    id: 'pressure_sensor',
    ...
    category: 'sensor',
    severity: 'warning',
    symptoms: ['Индикация датчика давления не горит в приложении', 'Компрессор не включается', 'Отсутствие сигнала в цепи'],
    diagnosis: 'Подключить приложение для диагностики. Проверить наличие сигнала от датчика давления. При исправном датчике должна гореть индикация.',
    solution: [
      'Проверить электрическое подключение датчика',
      'Проверить давление в системе манометрической гребенкой',
      'Проверка сигнала в блоке БСКУ'
    ],
    tools: ['Мультиметр', 'Манометрическая гребенка', 'Шуруповерт', 'Плата сигнальная']
  },
];
